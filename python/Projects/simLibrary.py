  ...
  for rows in range(len(map)):   
    for cols in range(len(map[rows])):
      match map[rows][cols]:
        ...
        case "r":
          print(greSeq, "s", end= endSeq)
        case "v":
          print(bluSeq, "v", end=endSeq)
        case _: #source: https://stackoverflow.com/questions/68804209/how-to-do-an-else-default-in-match-case
...
def findRecovered(regionMap, day, threshold, infectionDate): #checks day of infection, infectionMap, and checks cells for passed threshold time.
  for i in range(len(infectionDate)):
    for j in range(len(infectionDate[i])):
...
def updateMap(regionMap, threshold, day, infectionDate, mapLength, mapWidth):
  currentInfected = []
...
def simulate(regionMap, threshold, infectiousCount, susceptibleCount, recoveredCount, day, infectionDate):
  ...
  while(infectiousCount[day] != 0):
    ...
    regionMap, infectionDate = updateMap(regionMap, threshold, day, infectionDate, mapLength, mapWidth)
    day += 1
